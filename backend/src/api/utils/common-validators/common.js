 ...
 */
var applyAndAndOrOperators = function (schema) {
    return schema.merge(zod_1.z.object({
        $and: zod_1.z.lazy(function () { return schema.array(); }).optional(),
        $or: zod_1.z.lazy(function () { return schema.array(); }).optional(),
    }));
};
...
function recursivelyNormalizeSchema(transform) {
    return function (data) {
        var normalizedData = transform(data);
        Object.keys(normalizedData)
            .filter(function (key) { return ["$and", "$or"].includes(key); })
