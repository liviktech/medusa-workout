...
function cleanResponseData(data, fields) {
    ...
    if (!fields.length) {
        arrayData = arrayData.map(function (record) { return (0, utils_1.omitDeep)(record, EXCLUDED_FIELDS); });
        return (isDataArray ? arrayData : arrayData[0]);
    }
    var fieldsSet = new Set(__spreadArray(__spreadArray([], fields, true), INCLUDED_FIELDS, true));
    fields = __spreadArray([], fieldsSet, true);
